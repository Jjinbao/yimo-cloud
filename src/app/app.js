        ...
        function ($rootScope, $scope, $ionicHistory, $ionicViewSwitcher, $ionicPopup, $timeout, userService, $location, $state, $interval,$http) {
            ...
            connectWebViewJavascriptBridge(function (bridge) {
                bridge.registerHandler('onBack', function (response) {
                    ...
                    if(stateId.indexOf('userinfo')>=0||stateId.indexOf('history')>=0||stateId.indexOf('helpAnFeed')>=0||stateId.indexOf('videoDetail')>=0){
                        bridge.callHandler('backToApp', null, function (response) {

                      })
                    }else if(stateId.indexOf('infoDetail')>=0&&$location.url().indexOf('/info/list')>=0){
                        bridge.callHandler('backToApp', null, function (response) {
